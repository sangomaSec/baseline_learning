...
# options for MoCo v2  暂时不管这一part
...
def main():
    ...
    if args.multiprocessing_distributed:
        ...
        mp.spawn(main_operate, nprocs=ngpus_per_node, args=(ngpus_per_node, args))
    else:
        # Simply call main_worker function
        main_operate(args.gpu, ngpus_per_node, args)


def main_operate(gpu, ngpus_per_node, args):
    args.gpu = gpu
    ...
    train_dataset = datasets.ImageFolder(
        train_dir,
        moco.loader.TwiceCrops(transforms.Compose(augmentation))
    )
...
def accuracy(output, target, top_k=(1,)):
    """
    compute the accuracy of top k predictions
    :param output:
    :param target:
    :param top_k:
    :return:
    ...
    _, pre = output.top_k(max_k, 1, True, True)
    pre = pre.t()
    correct = pre.eq(target.view(1, -1).expand_as(pre))
